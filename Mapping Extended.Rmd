...
harveyPlot <- ggplot() + 
  ...
  geom_polygon(data=harvey, aes(x = long, y = lat, group = group, fill = federalShare))+
...
harveyPlot <- ggplot() + 
  geom_polygon(data=MainStates, aes(x=long, y=lat, group=group),colour="black",fill="white")+
  geom_polygon(data=harvey, aes(x = long, y = lat, group = group, fill = federalShare), color = "transparent")+
  labs(fill="Federal Aid Amount (USD)") + 
  ggtitle("Harvey-2017") +
  theme(plot.title = element_text(hjust = 0.5))
