...
author: "MA 615 Group 3: Bai,Ziyi;Jiang,Yuelin;Huang,Zijie;Zeng,Yuxin (Some of our code was inspired by Meng,Chenghao"
date: "2020/10/25"
...
```{r}
## filter target data
TrackFloyd <- hurr_tracks %>% dplyr:: filter(storm_id=="Floyd-1999")
TrackAllison <- hurr_tracks %>% dplyr:: filter(storm_id=="Allison-2001")
```
...
```{r}
## further adjustment on data
RainFloyd2 <- merge(RainFloyd1,CountyFips,by="fips") %>% separate(polyname, into= c("region","subregion"),sep= ",")
RainAllison2 <- merge(RainAllison1, CountyFips, by="fips") %>% separate(polyname, into=c("region","subregion"),sep=",")
```
...
##final dataset
## Floyd-1999
RainFloyd <- merge(MainStates,RainFloyd2,by=c("region","subregion"))
RainFloyd[RainFloyd==0.0] <- 0
# Make cut
RainFloyd$rain_cut <- cut(RainFloyd$sum_precip,breaks=c(0,25,50,75,100,125,150,175,200,225))

## Allison-2001
RainAllison <- merge(MainStates,RainAllison2,by=c("region","subregion"))
